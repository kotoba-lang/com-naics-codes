(ns naics-codes.main
  "Kotodama WASM entrypoint for the naics_codes clean-room actor (L4) — Clojure port.
  ...
  facts (`naics_codes.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "naics_codes")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"naics_codes.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "naics_codes-compat" :tier tier :entities entities} 200])
...
(defn start! [] :naics_codes-compat/ready)
